...
export function visitArrayWrapper(state: SerializeParameterState, node: Ast.TArrayWrapper): void {
    const parent: Ast.TNode = PQP.Parser.NodeIdMapUtils.assertUnboxParentAst(state.nodeIdMapCollection, node.id);
...
function visitArrayWrapperDefault(state: SerializeParameterState, node: Ast.TArrayWrapper): void {
...
function visitArrayWrapperForSectionMembers(
    ...
    node: Ast.IArrayWrapper<Ast.SectionMember>,
): void {
...
function visitArrayWrapperForUnaryExpression(
    state: SerializeParameterState,
    node: Ast.IArrayWrapper<Ast.IConstant<PQP.Language.Constant.UnaryOperator>>,
): void {
    // `not` is an unary operator which needs to be padded.
    // The default Any write kind is fine for the other operators (`+` and `-`).
    const elements: ReadonlyArray<Ast.IConstant<PQP.Language.Constant.UnaryOperator>> = node.elements;
    ...
    for (let index: number = 1; index < numElements; index += 1) {
        const operatorConstant: Ast.IConstant<PQP.Language.Constant.UnaryOperator> = elements[index];
