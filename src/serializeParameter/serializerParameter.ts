...
const DefaultWorkspace: TraverseWorkspace = {
    maybeWriteKind: SerializerWriteKind.Any,
    maybeIndentationChange: undefined,
};

function visitNode(state: SerializeParameterState, node: PQP.Language.Ast.TNode): void {
    switch (node.kind) {
        ...
        case PQP.Language.Ast.NodeKind.Csv: {
            const workspace: TraverseWorkspace = getWorkspace(state, node);
        ...
        case PQP.Language.Ast.NodeKind.ErrorRaisingExpression: {
            ...
            let pairedWorkspace: TraverseWorkspace;
        ...
        case PQP.Language.Ast.NodeKind.FieldSpecification: {
            ...
            if (maybeFieldTypeSpecification) {
                ...
                let typeWorkspace: TraverseWorkspace;
        ...
        case PQP.Language.Ast.NodeKind.FieldSpecificationList: {
            ...
            if (node.maybeOpenRecordMarkerConstant) {
                ...
                let workspace: TraverseWorkspace;
        ...
        case PQP.Language.Ast.NodeKind.FunctionExpression: {
            ...
            let expressionWorkspace: TraverseWorkspace;
        ...
        case PQP.Language.Ast.NodeKind.FunctionType: {
            ...
            const commonWorkspace: TraverseWorkspace = {
                maybeWriteKind: SerializerWriteKind.PaddedLeft,
            };
        ...
        case PQP.Language.Ast.NodeKind.ItemAccessExpression: {
            ...
            let closeWrapperConstantWorkspace: TraverseWorkspace;
        ...
        case PQP.Language.Ast.NodeKind.MetadataExpression: {
            ...
            let otherWorkspace: TraverseWorkspace;
            if (isMultiline) {
                otherWorkspace = {
                    maybeWriteKind: SerializerWriteKind.Indented,
                };
        ...
        case PQP.Language.Ast.NodeKind.TableType: {
            ...
            let rowTypeWorkspace: TraverseWorkspace;
...
function getWorkspace(
    ...
    node: PQP.Language.Ast.TNode,
    fallback: TraverseWorkspace = DefaultWorkspace,
): TraverseWorkspace {
    const maybeWorkspace: TraverseWorkspace | undefined = state.workspaceMap.get(node.id);
...
function propagateWriteKind(
    state: SerializeParameterState,
    parent: PQP.Language.Ast.TNode,
    firstChild: PQP.Language.Ast.TNode,
): void {
    const workspace: TraverseWorkspace = getWorkspace(state, parent);
...
function visitKeyValuePair(state: SerializeParameterState, node: PQP.Language.Ast.TKeyValuePair): void {
    const isMultilineMap: IsMultilineMap = state.isMultilineMap;
    ...
    let equalWorkspace: TraverseWorkspace;
    ...
    let valueWorkspace: TraverseWorkspace;
    if (valueIsMultiline) {
        valueWorkspace = {
            maybeIndentationChange: 1,
            maybeWriteKind: SerializerWriteKind.Indented,
...
function visitArrayWrapper(state: SerializeParameterState, node: PQP.Language.Ast.TArrayWrapper): void {
    const isMultiline: boolean = expectGetIsMultiline(state.isMultilineMap, node);
...
function visitIfExpression(state: SerializeParameterState, node: PQP.Language.Ast.IfExpression): void {
    ...
    let conditionWorkspace: TraverseWorkspace;
    let thenConstantWorkspace: TraverseWorkspace;
    ...
    let falseExpressionWorkspace: TraverseWorkspace;
...
function visitWrapped(state: SerializeParameterState, wrapped: PQP.Language.Ast.TWrapped): void {
    ...
    let workspace: TraverseWorkspace = getWorkspace(state, wrapped);
...
function wrapperOpenWriteKind(state: SerializeParameterState, wrapped: PQP.Language.Ast.TWrapped): SerializerWriteKind {
